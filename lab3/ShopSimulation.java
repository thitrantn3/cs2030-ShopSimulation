...
class ShopSimulation extends Simulation {
    ...
    public ShopSimulation(Scanner sc) {
        ...
        int l = sc.nextInt(); //maximum allowed length of the counter's queue.
        int m = sc.nextInt(); //maximum allowed length of the entrance/shop's queue

        //creating the counters array
        //and initializing each counter
        counters = new Counter[numOfCounters];
        for (int i = 0; i < numOfCounters; i++) {
            counters[i] = new Counter(i, true, new Queue(l));
        }
