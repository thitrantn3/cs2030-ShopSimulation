...
- Deadline: 7 June, 2021, Monday, 23:59, SST
- Mark: 3%
...
Copy your implementation of `cs2030.fp.Maybe` over before you get started with Lab 6.  A skeleton for for `Lazy<T>` is provided.  A wrongly implemented copy of `LazyList.java` is provided.
...
You are required to design a single `Lazy` class as part of the `cs2030.fp` package with two fields.  _You are not allowed to add additional fields_ to `Lazy`.
...
$ javadoc -quiet -private -d docs cs2030/fp/Lazy.java
```
...
You can test your code by running the `Test3.java` provided.  The following should compile without errors or warnings.  Make sure your code follows the CS2030 Java style and can generate the documentation without error.
```
$ javac cs2030/fp/*java
...
We have provided an interface called `Combiner<S, T, R>` in `cs2030.fp`, with a single `combine` method to combine two values, of type `S` and `T` respectively, into a result of type `R`.
...
You should make sure that your code follows the [given Java style guide](https://nus-cs2030-2021-s3.github.io/notes/style.html) and the [given Java documentation guide](https://nus-cs2030-2021-s3.github.io/notes/javadoc.html).
