...
class BusService {
  ...
  public CompletableFuture<Set<BusStop>> getBusStops() {
    return BusAPI.getBusStopsServedBy(serviceId).thenApply(this::returnBusStops);
  }

  private Set<BusStop> returnBusStops(String busStopsList) {
    Scanner sc = new Scanner(busStopsList);
  ...
  public CompletableFuture<Set<BusStop>> findStopsWith(String name) {
    return getBusStops().thenApply(stops -> filterStops(stops,name));
  }

  private Set<BusStop> filterStops(Set<BusStop> busStops, String name) {
    return busStops
       .stream()
