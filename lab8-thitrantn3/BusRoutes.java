...
class BusRoutes {
  ...
  public CompletableFuture<String> description() {
    ...
    for (BusService service : services.keySet()) {
      CompletableFuture<Set<BusStop>> stopsFuture = services.get(service);
      result = result.thenCombine(stopsFuture.thenApply(stops -> 
            describeService(service, stops).join()), (x,y) -> x + y);
    }
    return result;
  }
